...
export const getBookByIdController = async (req, res, next) => {
  try {
    const { id } = req.params;
    const book = await getBookByIdService(id, req.user);
    successResponse(
      {
        success: true,
...
export const getAllBooksController = async (req, res, next) => {
  try {
    ...
    const { genre, isDiscovery, limit, search, source, page } = req.query;
    ...
    console.log(
      `[API] <= /books returns ${books?.length} books out of ${total}`,
    );
    ...
    successResponse(
      {
        success: true,
        message: genre
          ? `Books for genre ${genre} fetched successfully`
        ...
        pagination: {
          ...
          totalPages,
          page: currentPage,
          limit: parseInt(limit, 10) || books.length,
        },
      },
      res,
    );
